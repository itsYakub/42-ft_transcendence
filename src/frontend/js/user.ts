import { navigate, showAlert } from "./index.js";export function userFunctions() {
    ...
    if (userForm) {
        userForm.addEventListener("submit", async (e) => {
            ...
            const password = userForm.password.value;            if ("loginButton" == e.submitter.id)
    ...
    });    const payloadJson = await response.json();    if (payloadJson.totpEnabled) {
        const totpCodeDialog = <HTMLDialogElement>document.querySelector("#totpCodeDialog");
        if (totpCodeDialog)
            totpCodeDialog.showModal();        const totpCodeForm = <HTMLFormElement>document.querySelector("#totpCodeForm");
        if (totpCodeForm) {
            totpCodeForm.code.addEventListener("keydown", (e: any) => {
                ...
                if (json.error) {
                    ...
                    return;
                }                console.log("user totp", json);                navigate("/");
            });
        }
    }
    ...
    document.cookie = `language=english; expires=${date}`;    navigate("/");
