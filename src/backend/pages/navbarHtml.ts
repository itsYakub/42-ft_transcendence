...
export function navbarHtml({ user, language }): string {
	...
	if (loggedIn)
		...
		html += loggedOutString(languageSelect) + loginString() + registerString() + totpSting();
			...
			<form id="totpCodeForm">
				...
				<div class="grid grid-cols-2 justify-between my-4">
					...
					<button type="submit" formmethod="post"
						class="ml-auto cursor-pointer text-white hover:bg-gray-700 bg-gray-800 border border-gray-700 font-medium rounded-lg px-2 py-2">%%NAVBAR_TOTP_CODE_VERIFY_TEXT%%</button>
				</div>
			</form>
		</div>
	</dialog>
