...
function gamerHtml(gamer: Gamer, isGamer1: boolean) {
	...
	<div class="flex flex-col gap-3">
		...
		<div class="w-60 py-2 mt-2 border ${colour} border-fuchsia-800 rounded-lg text-gray-900 text-center">${gamer.nick}</div>
	</div>
	`;
}
...
function spinnerHtml(gamer: Gamer[]) {
	...
	<div class="text-gray-300 text-5xl mx-8 pb-20 my-auto">vs</div>`
		:
		`
	<div class="my-auto pb-20">
		<svg aria-hidden="true" class="w-8 h-8 text-center animate-spin text-gray-900 fill-fuchsia-800" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
